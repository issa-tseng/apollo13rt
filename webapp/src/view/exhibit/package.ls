...
| \lifecycle => new Graphic( src: 'assets/lifecycle.svg', height: 700, caption_number: 1, caption: 'Apollo lifecycle. The S-IVB is still attached in Earth orbit (1), and is jettisoned once the spacecraft is in trans-lunar coast (2). Once in lunar orbit (3), the LM is sent and returns before it is dropped for trans-earth phase (4). The SM is ditched just before entry interface.' )
